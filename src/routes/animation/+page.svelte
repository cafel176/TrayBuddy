...
<script lang="ts">
   ...
   * 页面初始化核心逻辑
   *
   * 这是一个复杂的引导过程，确保所有多媒体组件同步就绪：
   * 
   * 1. **窗口设置**：启用鼠标穿透（默认），使宠物不遮挡后面应用的点击。
   * 2. **环境探测**：查询内存调试状态和后端构建模式。
   * 3. **资源初始化**：
   *    - 获取 Mod 全局配置（如 Canvas 层级、HUD 是否显示）。
   *    - 获取用户持久化设置（如昵称、音量、透明度）。
   * 4. **组件装载**：
   *    - 实例化 `SpriteAnimator`（角色层与边框层）。
   *    - 实例化 `AudioManager`（负责加载和播放音频资源）。
   *    - 实例化 `TriggerManager`（负责将前端交互转化为后端事件）。
   * 5. **事件注册**：建立与后端 Rust 层的双向监听。
   * 6. **初始渲染**：查询当前状态并执行首帧绘制。
   */
  async function init() {
    try {
      // 默认启用窗口级鼠标穿透
