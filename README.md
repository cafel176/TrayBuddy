...
- **桌面角色显示** — 透明无边框窗口，角色始终在桌面最上层
- **鼠标穿透** — 像素级透明区域自动穿透，不影响桌面操作
- **拖拽移动** — 自由拖拽角色到桌面任意位置
- **托盘吸附** — 角色自动吸附到系统托盘区域附近
- **对话气泡** — 打字机逐字显示效果、简易 Markdown 渲染、分支对话选项、自定义气泡样式
- **语音播放** — 状态切换时可播放关联的语音文件

#### 环境感知

- **媒体监听** — 通过 Windows GSMTC + Core Audio API 检测系统媒体播放状态
- **进程检测** — 识别工作应用（IDE、Office、设计工具等），自动触发对应事件
- **全屏检测** — 检测到全屏应用时自动进入免打扰模式
- **锁屏/解锁** — 通过 WTS 会话 API 检测屏幕锁定状态
- **天气与地理** — 基于 IP 定位获取天气信息，支持天气/气温条件触发
- **时间感知** — 日期、时间段、季节、节日、生日等时间维度触发
